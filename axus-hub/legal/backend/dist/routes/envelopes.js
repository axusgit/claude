...
export async function envelopeRoutes(app) {
    ...
    app.get("/", async (req, reply) => {
        ...
        if (!id)
            return;
        const { rows } = await pool.query(`select id, title, status, created_by, created_at, sent_at, completed_at
    ...
    app.get("/:id", async (req, reply) => {
       ...
       from recipient where envelope_id = $1 order by sign_order`, [envId]);
        ...
        const events = await pool.query(`select actor, type, detail, at from event where envelope_id = $1 order by at`, [envId]);
        return {
            envelope: env.rows[0],
    ...
    app.get("/:id/document", async (req, reply) => {
        ...
        if (!env.rowCount)
            return reply.code(404).send({ error: "Not found" });
        const fileName = env.rows[0].pdf_file ?? env.rows[0].source_file;
        if (!fileName)
