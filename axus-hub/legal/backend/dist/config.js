...
export const config = {
    ...
    databaseUrl: process.env.DATABASE_URL ?? "postgresql://axus:axus@localhost:5432/legal",
    platformDomain: process.env.PLATFORM_DOMAIN ?? "hub.axustechnologies.com",
    // Public origin for building signer links in emails.
    publicBaseUrl: process.env.PUBLIC_BASE_URL ?? "https://legal.hub.axustechnologies.com",
    // Where uploaded originals + sealed PDFs live (a mounted volume in prod).
    storageDir: process.env.STORAGE_DIR ?? "/data/legal",
    ...
    dev: {
        ...
        groups: process.env.DEV_USER_GROUPS ?? "app-legal|legal-role-admin",
    },
};
