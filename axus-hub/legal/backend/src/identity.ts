...
export type LegalRole = (typeof ROLES)[number];

export function legalRole(id: Identity): LegalRole | null {
  for (const r of ROLES) if (id.groups.includes(`legal-role-${r}`)) return r;
  return null;
}
