...
export async function runReminders(): Promise<number> {
  const { rows } = await pool.query(
    `select id, title, reminder_interval, sent_at, last_reminded_at
     from envelope
  ...
  for (const env of rows) {
    const iv = INTERVAL_MS[env.reminder_interval as string];
    if (!iv) continue;
    const base = env.last_reminded_at ?? env.sent_at;
    if (!base) continue;
    if (now - new Date(base).getTime() < iv) continue;
