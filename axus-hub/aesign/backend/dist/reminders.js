...
export async function runReminders() {
    ...
    const toPurge = await pool.query(`select id, title, source_file, pdf_file, sealed_file from envelope
     where deleted = true and deleted_at is not null and deleted_at < now() - ($1 || ' days')::interval`, [RECYCLE_DAYS]);
    for (const row of toPurge.rows) {
        ...
        await pool.query(`insert into activity (actor, action, detail, envelope_id) values ('system', 'Auto-purged from Recycle Bin', $1, $2)`, [row.title, row.id]);
    }
    ...
    const expired = await pool.query(`update envelope set status = 'expired'
     ...
     returning id, title`, [EXPIRE_DAYS]);
    for (const row of expired.rows) {
        await pool.query(`insert into event (envelope_id, actor, type, detail) values ($1, 'system', 'expired', $2)`, [row.id, `Auto-voided after ${EXPIRE_DAYS} days unsigned`]);
        // Awaited (not fire-and-forget) since this runner exits right after.
        await pool.query(`insert into activity (actor, action, detail, envelope_id) values ('system', 'Expired document', $1, $2)`, [row.title, row.id]);
    }
