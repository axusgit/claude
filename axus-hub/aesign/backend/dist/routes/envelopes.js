...
export async function envelopeRoutes(app) {
    ...
    app.get("/:id/document", async (req, reply) => {
        ...
        if (!id)
            return;
        const envId = req.params.id;
        const env = await pool.query(`select pdf_file, source_file, sealed_file, status, doc_type, title, company, quote_data
       from envelope where id = $1`, [envId]);
        if (!env.rowCount)
            return reply.code(404).send({ error: "Not found" });
        const row = env.rows[0];
    ...
    app.put("/:id/recipients", async (req, reply) => {
        ...
        try {
            ...
            if (ids.length) {
                await client.query(`delete from recipient where envelope_id = $1 and role <> 'viewer' and not (id = any($2::uuid[]))`, [envId, ids]);
            }
            else {
    ...
    app.post("/:id/send", async (req, reply) => {
        ...
        if (!signers.length)
            return reply.code(400).send({ error: "Add at least one signer." });
        ...
        if (viewers.length) {
            await pool.query(`update recipient set status = 'viewer', sign_token = null where envelope_id = $1 and role = 'viewer'`, [envId]);
        }
        await pool.query(`update envelope set status = 'sent', sent_at = now() where id = $1`, [envId]);
        await pool.query(`insert into event (envelope_id, actor, type, detail) values ($1, $2, 'sent', $3)`, [envId, id.email, `Sent to ${signers.length} signer(s)${viewers.length ? ` · ${viewers.length} copy recipient(s)` : ""}`]);
    ...
    app.post("/:id/send-copy", async (req, reply) => {
        ...
        const envq = await pool.query(`select id, title, status, pdf_file, source_file, sealed_file, doc_type, company, quote_data
       from envelope where id = $1`, [envId]);
        if (!envq.rowCount)
            return reply.code(404).send({ error: "Not found" });
        const e = envq.rows[0];
        const fileName = e.status === "completed" && e.sealed_file ? e.sealed_file : (e.pdf_file ?? e.source_file);
        if (!fileName)
            return reply.code(400).send({ error: "This document has no PDF to send yet." });
        const full = join(config.storageDir, fileName);
        if (!existsSync(full))
        ...
        await pool.query(`insert into event (envelope_id, actor, type, detail) values ($1, $2, 'copy_sent', $3)`, [envId, id.email, `Emailed a copy to ${valid.length} recipient(s): ${valid.join(", ")}`]);
    ...
    app.put("/:id/cc", async (req, reply) => {
        ...
        try {
            ...
            await client.query(`delete from recipient where envelope_id = $1 and role = 'viewer'`, [envId]);
            for (const r of list) {
                await client.query(`insert into recipient (envelope_id, name, email, role, sign_order, status)
           values ($1, $2, $3, 'viewer', 0, 'viewer')`, [envId, r.name, r.email]);
            }
            await client.query("commit");
        }
        catch (err) {
        ...
        const rows = await pool.query(`select id, name, email, role, sign_order, status from recipient
       where envelope_id = $1 and role = 'viewer' order by name`, [envId]);
