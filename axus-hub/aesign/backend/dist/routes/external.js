...
export async function externalRoutes(app) {
    ...
    app.post("/quotes", async (req, reply) => {
       ...
       values ($1, $2, 'Quote', $3, $4, $5) returning id`, [title, createdBy, q.customer.company.trim(), JSON.stringify(q), JSON.stringify(layout)]);
        ...
        await pool.query(`insert into event (envelope_id, actor, type, detail) values ($1, $2, 'created', $3)`, [envId, createdBy, `${title} (via ${createdBy})`]);
       ...
       values ($1, $2, $3, 'signer', 1) returning id`, [envId, recipName, recipEmail || "unknown@example.com"]);
