...
export const config = {
  ...
  databaseUrl:
    process.env.DATABASE_URL ?? "postgresql://axus:axus@localhost:5432/aesign",
  platformDomain: process.env.PLATFORM_DOMAIN ?? "hub.axustechnologies.com",
  // Public origin for building signer links in emails.
  publicBaseUrl:
    process.env.PUBLIC_BASE_URL ?? "https://aesign.axustechnologies.com",
  ...
  // Where to POST when an On Call-originated quote is completed (signed). Empty =
  // callback off. Reuses EXTERNAL_API_TOKEN as the bearer (same shared secret).
  onCallCallbackUrl: process.env.ON_CALL_CALLBACK_URL ?? "",
  mail: {
