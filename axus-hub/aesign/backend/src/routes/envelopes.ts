...
export async function envelopeRoutes(app: FastifyInstance) {
  ...
  app.post("/:id/apply-template", async (req, reply) => {
    ...
    if (!tmplPath) return { ok: true, applied: false, reason: "template file not found" };
  ...
  app.post("/:id/recipients/:rid/remind", async (req, reply) => {
    const id = requireStaff(req, reply);
    if (!id) return;
    const { id: envId, rid } = req.params as { id: string; rid: string };
    ...
    const recq = await pool.query(
      `select id, name, email, status, sign_token from recipient where id = $1 and envelope_id = $2`,
      [rid, envId],
    );
    if (!recq.rowCount) return reply.code(404).send({ error: "Recipient not found" });
    const r = recq.rows[0];
    if (r.status === "signed") {
