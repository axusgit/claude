...
export function renameActivity(envelopeId: string, newTitle: string): void {
  pool
    .query(
      `update activity set detail = $1
       where envelope_id = $2 and action in ('Created document', 'Created quote')`,
      [newTitle, envelopeId],
    )
    .catch(() => {});
}
...
export async function activityRoutes(app: FastifyInstance) {
  app.get("/", async (req, reply) => {
    ...
    const rows = search
      ? await pool.query(
          `select id, at, actor, action, detail from activity
