...
export async function quoteRoutes(app: FastifyInstance) {
  ...
  app.post("/", async (req, reply) => {
    ...
    const env = await pool.query(
      `insert into envelope (title, created_by, doc_type, company, quote_data)
       values ($1, $2, 'Quote', $3, $4) returning id`,
      [title, id.email, q.customer.company.trim(), JSON.stringify(q)],
    );
  ...
  app.put("/:id", async (req, reply) => {
    ...
    await pool.query(
      `update envelope set quote_data = $1, company = $2, source_file = $3, pdf_file = $3,
              title = coalesce($4, title) where id = $5`,
      [JSON.stringify(q), q.customer.company.trim(), fname, body.title?.trim() || null, envId],
    );
    const updated = await pool.query(`select * from envelope where id = $1`, [envId]);
