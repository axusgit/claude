...
// Built on pdf-lib to match slapdf.ts / cocpdf.ts (same fonts, palette, letterhead, and
// helpers). Like the SLA and Certificate of Completion, the BAA is now GENERATED ON THE FLY
// with per-customer values BAKED IN (Effective Date, Covered Entity name, governing-law
// state) — no stored template, no fragile stamp anchors. Axus is pre-identified as the
// Business Associate; the customer signs as the Covered Entity.
//
// The Axus letterhead is drawn full-page as the background of EVERY page. The two signature
// blocks sit SIDE BY SIDE on a DEDICATED FINAL PAGE at fixed positions, so the returned
// layout is deterministic and the frontend can hardcode a matching BAA_LAYOUT.
...
export interface BaaData {
  company?: string; // Covered Entity legal name (baked in)
  dateLong?: string; // Effective Date (baked in), e.g. "September 16, 2026"
  state?: string; // governing-law state (defaults to Florida)
}

type Block = { t: "h" | "p"; s: string };

function buildBody(coName: string, dateLong: string, state: string): Block[] {
  return [
    { t: "p", s: `This Business Associate Agreement (this "Agreement") is entered into as of ${dateLong} (the "Effective Date") by and between ${coName} ("Covered Entity") and Axus Technologies ("Business Associate"). Covered Entity and Business Associate are each a "Party" and together the "Parties."` },
    ...
    { t: "p", s: `6.5  Governing Law. This Agreement is governed by the laws of the State of ${state}, without regard to its conflict-of-laws provisions, except to the extent preempted by federal law.` },
...
// Today's date in Eastern Time, formatted like "September 16, 2026".
...
export async function generateBaaPdf(
  ...
  // Axus letterhead as a full-page background on every page, sized to fill the page exactly.
  ...
  const coName = d.company?.trim() || "________________________________________";
  ...
  const newPage = () => {
    ...
    y = TOP_SAFE; // first baseline below the letterhead's top logo band
  };
  ...
  // ---- Body ----
  for (const b of buildBody(coName, dateLong, state)) {
    if (b.t === "h") {
      ensure(46);
      y += 8;
  ...
  // how the body paginated (keeps the returned layout / hardcoded BAA_LAYOUT deterministic).
  newPage();
  ...
  const coHeading = d.company?.trim() ? d.company.trim() : "Covered Entity";
  const coveredEntitySlot = sigBlock("Covered Entity", coHeading, leftX);
  ...
  return { bytes: await pdf.save(), layout: [coveredEntitySlot, axusSlot] };
}
