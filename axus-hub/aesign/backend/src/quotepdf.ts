...
export async function generateQuotePdf(
  ...
  const drawItemsHeader = (pg: PDFPage, top: number): number => {
  ...
  cols.forEach((c, i) => {
    ...
    if (i > 0) p1.drawLine({ start: { x: M + i * colW, y: T(stripTop) }, end: { x: M + i * colW, y: T(stripTop + stripH) }, color: LINE, thickness: 0.5 });
  });
...
export async function generateQuoteTemplatePdf(): Promise<Uint8Array> {
  const blankRows: QuoteItem[] = Array.from({ length: 12 }, () => ({
    qty: 0,
    item: "",
    description: "",
    unit_price: 0,
