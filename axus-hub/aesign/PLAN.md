# Axus eSign — build plan

**Product:** `Axus eSign` — the home for all Axus legal activity: SOW/MSA signing,
quote generation & emailing, contract templates. Hub product tile at
`aesign.axustechnologies.com`, gated by Authentik SSO for staff; external
...
- **Packaging:** `axus-hub/aesign/` container(s) in `infra/docker-compose.yml`,
  Traefik route + `app-aesign` gate, like every other Hub product.
